...
const App = () => {
  ...
  // Asynchronous function to create a new Contentful entry and get its ID
  // Updated logic to find an existing entry instead of always creating a new one
  const createAndAddComponent = async (type: LayoutComponent['type']) => {
    try {
      const entryId = typeMap[type];

      // Find an existing entry of the correct type
      ...
      if (entries.items.length > 0) {
        // Use the ID of the first found entry
        contentfulId = entries.items[0].sys.id;
      } else {
        // If no published entry exists, create a new one
        const newEntry = await sdk.space.createEntry(entryId, {
          fields: {},
        });
      ...
      // Dispatch the addComponent action with a unique UI ID and the real Contentful entry ID
  ...
  React.useEffect(() => {
    ...
    const initialValue = sdk.entry.fields.layoutConfig.getValue();
    if (initialValue && Array.isArray(initialValue.components)) {
      // Assuming initialValue contains the new uiId and contentfulId
      dispatch(setLayout(initialValue));
    }
  }, [sdk, dispatch]);
          ...
          <Droppable droppableId="canvas-droppable">
            {(provided) => (
              <div {...provided.droppableProps} ref={provided.innerRef} className={styles.canvas}>
                {components.length > 0 ? (
                  components.map((component, index) => (
                    // Use the unique UI ID for the draggable key and ID
